...
//            getSimpleInfoHandle(simplePlayer,cmd);
    ...
    private void getNewMailNotify(MailTemplate mailTemplate) {
        ...
        MailProxy mailProxy = getProxy(ActorDefine.MAIL_PROXY_NAME);
        M16.M160007.S2C.Builder builder = M16.M160007.S2C.newBuilder();
        List<M16.MailShortInfo> res = new ArrayList<>();
        long id = mailProxy.createMail(mailTemplate);
        pushNewMail(id);
   ...
   //推送新邮件
    private void pushNewMail(long id) {
        MailProxy mailProxy = getProxy(ActorDefine.MAIL_PROXY_NAME);
        M16.M160007.S2C.Builder builder = M16.M160007.S2C.newBuilder();
        List<M16.MailShortInfo> res = new ArrayList<>();
        res.add(mailProxy.getReportMailShortInfoByid(id));
