...
public class WebSocketEncoder extends ProtocolEncoderAdapter{
    ...
    public void encode(IoSession session, Object message, ProtocolEncoderOutput out) throws Exception {
        ...
        if(message instanceof List){
            ...
            resultBuffer = IoBuffer.allocate(sendData.length);
            resultBuffer.setAutoExpand(true);
            resultBuffer.put(sendData);
            resultBuffer.flip();
            resultBuffer.free();

            isDataFramePacket = true;
        }
...
//            WebSocketCodecPacket packet = (WebSocketCodecPacket)message;
            resultBuffer = isRemoteWebSocket ? WebSocketEncoder.buildWSDataFrameBuffer(resultBuffer) : resultBuffer;
        }
        else{
    ...
    private static IoBuffer buildWSDataFrameBuffer(IoBuffer buf) {
        ...
        buffer.setAutoExpand(true);
        buffer.put((byte) 0x81);   //0x82二进制格式 0x81字符串格式
